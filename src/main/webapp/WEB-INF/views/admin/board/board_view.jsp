...
<style>
/* 아래 미디어쿼리는 IE10,11에서 지원하는 전용CSS 적용 */
	...
	$.ajax({
		...
		success:function(result) {
			if(typeof result=="undefined" || result=="" || result==null){
				$("#collapseReply").empty();//div태그 내용 삭제.
				...
				//JSON.parse(문자열) - 일반문자열을 json으로 변경해주는 함수
		...
		$.ajax({
			...
			success:function(result) {
				if(result=="success") {
					...
					$("#reply_page").val("1"); //댓글 삭제 후 1페이지로 이동 
					replyList();
				}
			},
			error:function() {
	...
	$("#btn_reply_write").click(function(){
		...
		$.ajax({
			...
			success:function(result){//댓글 입력이 성공시 실행 
				...
				replyList(); // 댓글 등록 후 바로 등록된 댓글 실시간으로 update
			},
			error:function() {
				alert("RestAPI서버가 작동하지 않습니다. 잠시 후 이용해 주세요.")
			}
