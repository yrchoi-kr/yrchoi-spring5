...
public class AdminController {
	...
	public String updateMember(HttpServletRequest request, MultipartFile file, MemberVO memberVO, PageVO pageVO) throws Exception {
		//프로필 이미지 처리 추가
		if(!file.getOriginalFilename().isEmpty()) {//신규파일이 있으면 실행 없으면 건너뜀.
	...
	public String deleteMember(HttpServletRequest request, MemberVO memberVO) throws Exception {
		...
		//DB 테이블 삭제 후 회원프로필 이미지가 exist() ==true면 삭제하는 로직 추가
	...
	//아래, @import방식 : 최신 게시물용도로 사용 //페이지 안에서 컴파일된 다른 페이지를 불러올 수 잇음.
	@RequestMapping(value="/admin/latest/latest_board",method=RequestMethod.GET)
	public String latest_board(@RequestParam(value="board_name", required=false)String board_name, @RequestParam("board_type")String board_type,Model model) throws Exception {
		...
		return "admin/latest/latest_board"; // jsp  생략, 최신 게시물을 출력하는 결과 페이지 생성
	}
}