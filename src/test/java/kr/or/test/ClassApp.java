 ...
 * 2. 오브젝트(객체)생성에 필요한 new 키워드(예약어) 생성자 메서드(함수) 사용 연습,
 * 3. 추상클래스(Abstract) 대한 연습
 * @author admin
 *
 */
//오브젝트생성에 필요한 new 키워드 생성자 메서드 사용하는 실습 클래스(아래)
class Circle {
	...
	//원의 넓이를 구하는 메서드 반환리턴값의 크기가 더블형(아래)
...
abstract class GraphicObject {
	...
	//추상메서드를 만드는 이유는 메서드가 수십개 수백개일때 개발자가 구현할때 어려움이 존재
...
class Traiangle extends GraphicObject {
	//삼각형 그리기
	@Override //부모클래스의 메서드를 재정의해서 사용 = 오버라이드; 면접에서 오버로드 오버라이드 구분 자주 나옴 Step1 오버로드 확인
	void draw() {
		// 삼각형을 만듬
		System.out.println("  *  ");
		System.out.println(" * * ");
...
public class ClassApp {
	...
	public static void main(String[] args) {
		...
		// 개발자가 입력한 반지름의 원의 넓이를 구하는 오브젝트를생성(아래)
