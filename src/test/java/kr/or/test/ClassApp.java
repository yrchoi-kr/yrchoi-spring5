package kr.or.test;
/**
 * 1. 이 클래스는 클래스 상속(extends)에 대한 연습,
 * 2. 오브젝트(객체)생성에 필요한 new 키워드(예약어) 생성자메서드 사용 연습,
 * 3. 추상클래스(Abstract) 대한 연습
 * @author 라건국의 DeskTop
 *
 */
//오브젝트 생성에 필요한 new 키워드 생성자 메서드 사용한 실습(아래)
class Circle {
	private int r;// r = 원의 반지름으로 사용할 변수
	//원(Circle)을 만드는 메서드(아래)	
	//public Circle() { } 만들지 않아도 자동으로 만들어집니다.
	public Circle(int radius) {
		r = radius; //Get|Set 중에 셋 저장처리
	}
...
class Salesman extends Employee { //extends확장(상속)
	public Salesman() { //클래스 명과 똑같은 이름의 메서드가 생성자 함수입니다.
		// 자동으로 생성되는데, 개발자가 커스터마이징 할때 만듭니다. (아래)
		mDept = "판매부서"; //부모클래스의 변수를 상속받아서 사용 가능
	}
	public void doJob() {
...
abstract class GraphicObject {
	...
	//추상메서드를 만드는 이유는 메서드가 수십,수백개 일때 개발자가 구현할때 어려움이 존재
	// 위 문제를 해소한 기능의 클래스 수백개의 메서드를 이름만 모아서, 유지 보수가 편리하게 처리한 방식
}
// 추상클래스를 사용하는 방법(아래)
// 스프링에서는 추상클래스보다는 인터페이스를 더 많이 사용합니다. 사용의도 비슷합니다.
class Traiangle extends GraphicObject {
	
	@Override // 부모클래스의 메서드를 재정의 해서 사용 = 오바라이드 / 오버로드 2개 차이점 확인(step1.java 확인)
	void draw() {
		// 삼각형을만듬
		System.out.println("  *  ");
		System.out.println(" *** ");
...
public class ClassApp {
	...
	public static void main(String[] args) {
		...
		// 개발자가 입력한 반지름의 원의 넓이를 구하는 오브젝트 생성(아래)
		Circle circle = new Circle(5);//반지름이 5인 원의 넓이를 구하는 객체 생성
		System.out.println("원의 넓이는 " + circle.getCircle());
		circle = null; //오브젝트(메모리) 초기화 (또는 반환)
		//메모리에 올라가지 않은 Circle는 못쓰고 올라간 circle은 사용가능
		//클래스 상속과 메서드 실행
