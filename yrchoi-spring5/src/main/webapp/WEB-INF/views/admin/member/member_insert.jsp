...
<script>
$(document).ready(function(){
	//RestAPI서버클래스에서 ID중복체크 메서드를 확인 합니다.
	//RestAPI클라이언트 맛보기
	$("#btn_id_check").click(function(){
		...
		$.ajax({
			...
			dateType:"text",//결과값(0,1)을 받을떄, 데이터형을 text, json, xml중 선택
			success:function(result){
				alert(result);
				if(result==0){//중복ID가 있다면 정상진행
					$("#btn_insert").attr("disabled",false);//등록버튼 활성화
					alert("사용 가능한 ID입니다.");
				}
				if(result==1){//중복ID가 없다면 진행중지
					$("#btn_insert").attr("disabled",true);//등록버튼 비활성화
					alert("중복ID가 존재합니다. 다시입력해주세요!");
				}
				if(result==2){//입력 ID가 없다면 진행중지
					$("#btn_insert").attr("disabled",true);//등록버튼 비활성화
					alert("ID를 입력하지 않았습니다.!");
				}
			},
			error:function(){
				alert('RestAPI서버가 작동하지않습니다.다음에 이용해 주세요!');
			}
		});
	});
