				...
				<fieldset>
					...
					<ul class="app_list">
						<li class="clear">
							...
							<div class="app_content"><input value="${memberVO.user_id}"type="text" name="user_id" class="w100p" id="user_id_lbl" placeholder="아이디를 입력해주세요" required/></div>
						</li>
						<li class="clear">
						...
						<li class="clear">
							...
							<div class="app_content"><input value="${memberVO.user_name}" type="text" name="user_name" class="w100p" id="user_name_lbl" placeholder="사용자이름을 입력해 주세요" required/></div>
						</li>
						<li class="clear">
							<label for="email_lbl" class="tit_lbl pilsoo_item">이메일</label>
							<div class="app_content"><input value="${memberVO.email}" type="email" name="email" class="w100p" id="email_lbl" placeholder="이메일을 입력해 주세요" required/></div>
						</li>
						<li class="clear">
							<label for="point_lbl" class="tit_lbl pilsoo_item">포인트</label>
							<div class="app_content"><input value="${memberVO.point}" type="digits" name="point" class="w100p" id="point_lbl" placeholder="포인트를 입력해주세요" required/></div>
						</li>
						<li class="clear">
					...
					<button class="btn_baseColor" id="btn_leave" style="cursor:pointer;">회원탈퇴</button>
					</p>	
				</fieldset>
			</form>
			<!-- //폼영역 -->
...
$(document).ready(function(){
	$("#btn_leave").click(function(){
		...
		//$("select[name='enabled']").html("<option valu='false'>탈퇴</option>"); //select set
		//alert($("select[name='enabled']").val()); //Get
		//위 값을 false, 0 둘중 1개 로 변경 후 submit예정.
		form_leave.attr("action","/member/mypage_leave");//크롬 호환성 추가
		form_leave.submit();//삭제는 아니고, enabled를 업데이트함
