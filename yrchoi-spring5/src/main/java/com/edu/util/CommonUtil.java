...
public class CommonUtil {
	...
	public void profile_upload(String user_id, HttpServletRequest request,MultipartFile file) throws IOException {
		...
		File makeFolder = new File(folderPath);//여기서 profile폴더가 만들어집니다.
		if(!makeFolder.exists()) {
			makeFolder.mkdir();//여기서 신규 폴더가 생성됩니다.
		}
		//1개의 파일에 1000개 이상이면, 조회속도 엄청느려짐.
		//년월폴더를 생성 후, 해당 년월에 업로드된 파일은 년월폴더로 관리.
		byte[] fileData = file.getBytes();
		File target = new File(makeFolder,user_id+".jpg");//user_id는 PK이기 때문에 기존파일이 있다면, 덮어쓰면서 저장됩니다.
		FileCopyUtils.copy(fileData, target);//첨부파일이 저장됩니다.
		
	}

	public void profile_delete(String user_id, HttpServletRequest request) throws IOException {
		// TODO 프로필 이미지가 프로필 폴더에 존재하면 삭제하는 로직
		String folderPath = request.getServletContext().getRealPath("/resources/profile");
		File target = new File(folderPath,user_id+".jpg");
			if(target.exists()) {
				target.delete();//프로필 파일이 실제로 지워짐.
			}
		}
}